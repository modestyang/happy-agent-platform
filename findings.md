...
- `CurrentGoalReportPage` 当前每次挂载都调用 `refreshCurrentGoalReport('USER_REFRESH')`，即无条件 POST；浏览器手势导致路由卸载/重挂载时会重复入队，服务端持久报告和 GET 入口没有被前端复用。
- 报告页返回按钮使用 `navigate('/')`，会再压入一条首页历史；改为 replace 才能避免“首页→报告→首页”继续堆叠。
- 现有后端已把客观事实和模型叙事分开：体重、训练量、部位、力量/有氧比例由 fitness service 确定性计算；模型只能生成结论、亮点、薄弱项和行动。可在不改数据库表的前提下扩充月度及数据覆盖度指标。
- 当前综合分数/等级由模型给出而非确定性公式，不适合在专业报告里作为首要数据；UI 应以统计周期、真实指标和可追溯趋势为主。
- WHO 成人基准为每周 150–300 分钟中等强度或 75–150 分钟高强度有氧，并至少 2 天主要肌群力量活动；本系统只有计划时长与标题/部位推断，没有强度证据，因此只能展示参考基准和差距所需数据，不能宣称用户已达标。
