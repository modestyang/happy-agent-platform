...
function ComponentDetail({ component, form, catalog, readOnly, preview, dirty, pending, error, success, onBack, onPreview, onUpdate, onUpdateConfig, onSave }: {
  ...
  const selectableToolKeys = [...toolKeys, ...requiredTools.filter((key) => !toolKeys.includes(key))];
  ...
  return <>
    ...
    {readOnly ? <ToolDetail component={component} catalog={catalog} /> : <div className="admin-detail-stack">
      ...
      {component.type === 'SKILL' && <>
        ...
        <section className="admin-detail-card"><header><div><h2>依赖工具</h2><p>仅可选择当前已启用的代码注册工具。</p></div></header><div className="admin-tool-chips">{selectableToolKeys.map((key) => { const unavailable = !toolKeys.includes(key); return <button key={key} className={`${requiredTools.includes(key) ? 'is-active' : ''}${unavailable ? ' is-unavailable' : ''}`} onClick={() => toggleTool(key)}><Check />{unavailable ? `未登记工具：${key}` : key}</button>; })}{!toolKeys.length && !requiredTools.length && <p>暂无可用工具。</p>}</div></section>
      </>}
