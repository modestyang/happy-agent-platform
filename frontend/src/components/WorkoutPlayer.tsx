...
import { createBrowserVoiceEngine, isVoiceSupported, voiceCueForTransition, type VoiceEngine } from '../workout/voiceGuidance';
...
export function WorkoutPlayer({ plan, exerciseLibrary, reload }: WorkoutPlayerProps) {
  ...
  useEffect(() => {
    if (!current) return;
    const cue = voiceCueForTransition(previousVoiceSession.current, session, current);
    previousVoiceSession.current = session;
    if (cue) voiceEngine.current?.speak(cue);
  }, [current, session]);

  useEffect(() => () => voiceEngine.current?.stop(), []);
  ...
  if (session.phase === 'READY') {
    return <section className="page workout-page workout-ready">
      <header className="workout-top"><button className="workout-top-button" aria-label="返回计划" onClick={() => navigate('/plan')}><ChevronLeft /></button><small>训练预览</small><button className="workout-top-button" aria-label={muted ? '打开声音' : '关闭声音'} onClick={() => setMuted((value) => { const next = !value; voiceEngine.current?.mute(next); return next; })}>{muted ? <VolumeX /> : <Volume2 />}</button></header>
      ...
      <button className="workout-start" onClick={() => { voiceEngine.current?.unlock(); started.current = true; navigator.vibrate?.([50, 40, 50]); setSession((value) => startWorkoutSession(value)); }}><Play /> 开始训练</button>
    </section>;
  }
  ...
  return <section className={`page workout-page${isRest ? ' workout-page--rest' : ''}`}>
    <header className="workout-top"><button ref={exitTrigger} className="workout-top-button" aria-label="退出训练" onClick={() => { voiceEngine.current?.stop(); pausedBeforeExit.current = paused; setPaused(true); setExitOpen(true); }}><X /></button><div className="workout-progress"><i style={{ width: `${progress}%` }} /></div><button className="workout-top-button" aria-label={muted ? '打开声音' : '关闭声音'} onClick={() => setMuted((value) => { const next = !value; voiceEngine.current?.mute(next); return next; })}>{muted ? <VolumeX /> : <Volume2 />}</button></header>
    {!voiceAvailable && <p className="workout-voice-note"><VolumeX /> 当前浏览器不支持语音播报，计时仍会正常继续。</p>}
    ...
    <div className="workout-controls"><button aria-label="上一个动作" disabled={session.exerciseIndex === 0} onClick={() => { voiceEngine.current?.stop(); setSession((value) => previousWorkoutSession(value)); }}><SkipBack /></button><button className="workout-pause" aria-label={paused ? '继续训练' : '暂停训练'} onClick={() => { const next = !paused; voiceEngine.current?.speak({ id: `control:${next ? 'pause' : 'resume'}:${voiceControlSequence.current++}`, text: next ? '训练暂停' : '继续训练', interrupt: false }); setPaused(next); }}>{paused ? <Play /> : <Pause />}</button><button aria-label="下一个动作" disabled={session.exerciseIndex === exercises.length - 1} onClick={() => { voiceEngine.current?.stop(); setSession((value) => nextWorkoutSession(value)); }}><SkipForward /></button></div>
