...
## 2026-08-14 专业目标报告与导航稳定性

- 基线已提交为 `034a59e feat(platform): improve agent workflows and mobile chat`；提交前 Maven 全量 reactor BUILD SUCCESS，前端拆组 116/116 + 21/21、类型检查、生产构建、契约 lint/生成幂等、Spotless、本地部署编排与 diff 检查通过。
- 已完成现有报告全链路审计：确认重复生成来自前端挂载时无条件 POST，返回按钮另行压入首页历史；后端确定性事实已有安全边界，无需修改数据库 migration。
- 已核对 WHO/CDC 成人身体活动指南；设计决定加入指南参考与“强度未采集、不可判定达标”的数据局限，不输出医疗诊断。
- 报告加载 TDD 已完成：普通进入和浏览器重入先 GET，只有明确 404 才 POST；返回首页使用 replace，导航测试证明不再增加 history index。
- 报告事实已增加当前目标周期内的本月训练次数/时长、身体记录数和饮食记录数；精确 Java 测试通过，未改数据库和响应结构，仅扩充现有指标 key 枚举并重新生成前端类型。
- 报告模型提示已要求所有判断引用命名指标，并把 WHO 活动量只作为参考；没有强度和力量训练日证据时必须说明不足，不得宣称达标或输出诊断。
- 前端专业简报已完成：隐藏模型给出的综合分、移除总卡片边框，以本月概览、趋势与结构、分析结论、行动建议、数据与方法五层呈现；关键正文 12–14px、指标值 24px，定向组件测试通过。
- 当前进入全量前后端、格式、契约和发布门禁；通过后提交并使用现有 ACR 流程部署。
