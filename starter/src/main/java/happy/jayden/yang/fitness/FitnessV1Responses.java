...
final class FitnessV1Responses {
  ...
  static Job job(MealRecognitionJobDto value) {
    Failure failure = value.failureCode() == null ? null : new Failure(
        "DEPENDENCY_NOT_CONFIGURED".equals(value.failureCode()) ? value.failureCode() : "TASK_FAILED",
        value.failureMessage() == null ? "识别失败" : value.failureMessage(),
        "DEPENDENCY_NOT_CONFIGURED".equals(value.failureCode()));
    return new Job(value.jobId(), value.status(), value.mediaId(), value.mealType().name(), value.occurredAt(),
        value.candidates(), failure, value.createdAt(), value.updatedAt());
  }

  static MealRecord meal(MealDto value) {
    double calories = value.items().stream().mapToDouble(MealItemDto::estimatedKcal).sum();
    return new MealRecord(value.id(), value.mealType().name(), value.occurredAt(), value.source(),
        value.recognitionJobId(), value.note(), value.items(), new Nutrition(calories, 0, 0, 0), value.occurredAt());
  }

  record Failure(String code, String message, boolean retryable) {}
  record Job(UUID jobId, String status, UUID mediaId, String mealType, Instant occurredAt,
             List<MealRecognitionCandidate> candidates, Failure failure, Instant createdAt, Instant updatedAt) {}
  record Nutrition(double caloriesKcal, double proteinG, double carbohydrateG, double fatG) {}
  record MealRecord(UUID mealRecordId, String mealType, Instant occurredAt, String source,
                    UUID recognitionJobId, String note, List<MealItemDto> items, Nutrition nutrition,
                    Instant createdAt) {}
}
