...
## 2026-08-14 专业目标报告与导航稳定性（当前阶段）

### Goal

修复报告页在浏览器手势往返时重复入队生成和历史栈循环的问题；把当前目标报告重做为无总卡片外框、移动端可读、只使用真实记录并明确科学依据与数据局限的专业分析页，验证后通过现有 ACR 发布链部署到 ECS。

### Phases

1. [complete] 提交上一批已部署并验证的 Tool/SAA、计划、对话体验和发布脚本改动。
2. [complete] 定位报告重入与重复生成根因，审计现有报告事实、叙事、API 和前端结构，并核对权威运动指南。
3. [complete] 按 TDD 修复读优先的报告加载与稳定返回语义，扩充月度/覆盖度客观指标。
4. [complete] 重做报告信息架构、字号、无外框布局、证据与数据局限展示。
5. [complete] 运行后端、前端、契约、架构与部署门禁，提交本批改动。
6. [in_progress] 通过 ACR 发布到 ECS，核对 App/Nginx 健康、PostgreSQL 不变和域名入口。

### Decisions

- 用户明确要求自行完成并部署，视为对 bounded 设计的预授权；不等待睡眠期间的逐项确认。
- 页面挂载先读取持久报告，仅在服务端明确返回 404 时创建；普通重入、浏览器前进/后退不得触发 POST。
- 报告不使用模型生成的分数作为主要结论；事实、趋势、覆盖度和建议层级分离，数据不足必须显式说明。
- 成人活动基准采用 WHO 的 150–300 分钟中等强度或 75–150 分钟高强度有氧、每周至少 2 天力量活动；当前系统未采集强度时不得判断是否达标。

### Errors Encountered

| Error | Attempt | Resolution |
|---|---:|---|
| `npm test` 在与 Maven/构建并行的整包 137 测试中出现两个 5 秒超时和两个管理端时序失败 | 1 | 不修改产品代码；按正式发布门禁拆分移动端 116/116 与管理端 21/21，串行复验均通过 |
| 生成 OpenAPI 类型后直接对 HEAD 执行 `git diff --exit-code` 会把本批有意生成变更当失败 | 1 | 生成前后比较两个生成文件 SHA-256，确认生成器幂等；契约 lint 110 项通过 |
| 报告返回按钮行为测试实际把 history index 从 1 增至 2 | 1 | 将返回首页改为 replace，避免手势往返形成重复历史链；回归测试转绿 |
| 新增月度/覆盖度指标和科学叙事约束后两个 Java 测试精确失败 | 1 | 确定性补齐当月训练/时长及身体/饮食记录数；提示词要求引用事实且强度不足时不得宣称达标 |
| 专业版式测试发现旧报告仍显示模型分数且总卡片边框为 1px | 1 | 隐藏非确定性评分，替换为无总外框的分节健康简报并统一可读字号；定向测试转绿 |
